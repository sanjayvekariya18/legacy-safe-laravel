            ...
            <div class="row input-row dpb-30">
                ...
                <div class="col-6 position-relative dmb-20">
                    <x-text-input class="white-b-input" type="text" name="mobile_number"
                        placeholder="Mobile Number (+44)" :value="old('mobile_number')" required autocomplete="mobile_number" />
                ...
                @foreach ($roles as $role)
                    <div class="d-flex align-items-center user-permission-check">
                        <div
                            ...
                            <input name="roles[]" value="{{ $role->id }}" type="checkbox" class="opacity-0 position-absolute top-0 start-0">
                            <span class="check-box black-checkbox radius5 position-relative me-3"></span>
                            {{ $role->name }}
                        </div>
                        <a href="#solicitor-modal" data-role="{{ $role->name }}" data-permissions="{{ json_encode($role->permissions->pluck('name')) }}"
                            data-bs-toggle="modal" data-bs-target="#solicitor-modal"
                            class="text-decoration-none d-inline-block tk-basic-sans fw-normal font16 leading30 space-0_16 text-808080 ms-1">(view
                            permissions)</a>
                    </div>
                @endforeach
            </div>
            <div class="d-flex justify-content-end dpb-80">
                <button type="submit"
    ...
    <script type="module">
        ...
        $('#solicitor-modal').on('show.bs.modal', function(event) {
            ...
            permissions.forEach(function(item) {
                listContainer.append(`<li class="dmb-15"><a href="" class="d-inline-block text-decoration-none tk-basic-sans font16 leading19 space-0_16 text-808080 fw-normal">• ${item}</a></li>`);
            });
        });
    </script>
@endpush
